...
const CACHE_NAME = "charterhouse-freeze-static-8H1";
const PRECACHE_URLS = [
  "./",
...
function scopeUrl(relative) { return new URL(relative, self.registration.scope).toString(); }
async function precacheAll() {
  const cache = await caches.open(CACHE_NAME);
  const requests = PRECACHE_URLS.map(relative => new Request(scopeUrl(relative), { cache: "reload" }));
  await cache.addAll(requests);
}
self.addEventListener("install", event => {
  event.waitUntil((async () => { await precacheAll(); await self.skipWaiting(); })());
});
  ...
  event.respondWith((async () => {
    ...
    try {
      ...
      if (request.mode === "navigate") {
        const fallbackName = url.pathname.endsWith("/leaderboard.html") ? "./leaderboard.html" : "./index.html";
...
self.addEventListener("message", event => {
  ...
  event.waitUntil((async () => {
    const cache = await caches.open(CACHE_NAME);
    const matches = await Promise.all(PRECACHE_URLS.map(relative => cache.match(scopeUrl(relative), { ignoreSearch: true })));
    const missing = PRECACHE_URLS.filter((_, index) => !matches[index]);
    const reply = { type: "OFFLINE_READY_RESULT", build: BUILD, ready: missing.length === 0, cachedItems: PRECACHE_URLS.length - missing.length, requiredItems: PRECACHE_URLS.length, missing };
